...
<body class="bg-gray-50 min-h-screen">
    <div class="container mx-auto px-4 py-6">
        ...
        <header class="flex flex-col md:flex-row justify-between items-center mb-8 border-b pb-4">
            ...
            <div class="text-right bg-white shadow-sm rounded-lg px-4 py-2 border border-gray-400 relative">
                <p class="text-sm <?php
                    $currentHour = (int)date('G');
                    if ($currentHour >= 6 && $currentHour < 12) {
                        echo 'text-primary-600';
                ...
                ?> flex items-center justify-end">
                    <i class="far fa-calendar-alt <?php
                        if ($currentHour >= 6 && $currentHour < 12) {
                            echo 'text-primary-500';
                        } elseif ($currentHour >= 12 && $currentHour < 17) {
                ...
                ?> flex items-center justify-end cursor-pointer" id="current-time">
                ...
                <!-- Greeting Toast Notification -->
                <div id="greeting-toast" class="absolute right-0 mt-2 z-50 bg-white border border-gray-300 shadow-lg rounded px-10 py-3 text-lg font-semibold text-gray-800 flex items-center space-x-4 transition-opacity duration-300 opacity-0 pointer-events-none">
                    <i class="fas fa-sun text-yellow-400" id="greeting-icon"></i>
                    <span id="greeting-message" class="whitespace-nowrap"></span>
                </div>
            </div>
        </header>
        ...
        <!-- Timesheet Records -->
                <?php
                // After executing $timesheet_stmt
                $has_records = isset($timesheet_stmt) && $timesheet_stmt->rowCount() > 0;
                ?>
                <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    <div class="p-4 bg-gray-50 border-b flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-center mb-2 sm:mb-0 px-6">
                            <i class="fas fa-table text-primary-600 mr-2" style="margin-bottom: -5px;"></i>
                            <h2 id="timesheet-records-title" class="text-2xl font-semibold text-gray-800">Timesheet Records</h2>
                        </div>
                            <?php if ($has_records): ?>
    ...
    include './components/modals/delete-empty-modal.php';
    include './components/modals/reset-empty-modal.php';
    include './components/modals/export-empty-modal.php';
    ?>
...
<script src="./assets/js/greeting-message.js"></script>
</html>