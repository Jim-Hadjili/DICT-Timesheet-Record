...
function setupOvertimeModal() {
  ...
  // Open overtime modal
  if (overtimeButton) {
    // Remove the interval that forcibly enables the button
    // Instead, base button state on actual requirements
    const checkAndEnableOvertimeButton = () => {
      // Get the PHP-set flag if it exists
      const overtimeEligible = document.getElementById("overtime-eligible");

      if (overtimeEligible && overtimeEligible.value === "1") {
        overtimeButton.removeAttribute("disabled");
        overtimeButton.classList.remove("opacity-50", "cursor-not-allowed");
      }
    };

    // Run once on load
    checkAndEnableOvertimeButton();
  ...
  if (confirmOvertime && overtimeForm) {
    confirmOvertime.addEventListener("click", function () {
      ...
      showCustomAlert("Overtime request submitted successfully", "success");

      // Submit the form after a short delay to let the user see the message
      setTimeout(() => {
        overtimeForm.submit();
      }, 1000);
    });
  ...
  document.addEventListener("keydown", function (e) {
    if (
      e.key === "Escape" &&
      overtimeModal &&
      !overtimeModal.classList.contains("hidden")
    ) {
      overtimeModal.classList.add("hidden");
      document.body.classList.remove("overflow-hidden");
    }
...
// Ensure the overtime button is always enabled
document.addEventListener("DOMContentLoaded", function () {
  const overtimeButton = document.getElementById("overtime-button");

  // Make sure the overtime button is always enabled
  if (overtimeButton) {
    // Remove any disabled attributes that might have been set
    overtimeButton.removeAttribute("disabled");

    // Ensure the button stays enabled by adding this event listener
    setInterval(function () {
      overtimeButton.removeAttribute("disabled");
    }, 1000);
  }

  // Original overtime button click handler
  if (overtimeButton) {
    overtimeButton.addEventListener("click", function () {
      const overtimeModal = document.getElementById("overtime-modal");
      if (overtimeModal) {
